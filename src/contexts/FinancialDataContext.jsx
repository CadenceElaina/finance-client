...
export const FinancialDataProvider = ({ children }) => {
    ...
    const [data, setData] = useState({ accounts: DEMO_ACCOUNTS, budget: DEFAULT_DEMO_BUDGET });
    const [persistence, setPersistence] = useState(() =>
        localStorage.getItem('financialDataPersistence') || (user ? 'server' : 'local')
    );
    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (user && persistence === 'server') {
                ...
                loaded = getLocalData() || { accounts: DEMO_ACCOUNTS, budget: DEFAULT_DEMO_BUDGET };
                setData(loaded);
            } else {
                setData({ accounts: DEMO_ACCOUNTS, budget: DEFAULT_DEMO_BUDGET });
            }
    ...
    const removeAccount = (accountId) => {
        saveData({ ...data, accounts: data.accounts.filter(acc => acc.id !== accountId) });
    };
    ...
    const addExpense = (newExpense) => {
        const updatedBudget = {
            ...data.budget,
            monthlyExpenses: [...data.budget.monthlyExpenses, { ...newExpense, id: `exp-${Date.now()}` }]
        };
        updateBudget(updatedBudget);
    };
    ...
    const removeExpense = (id) => {
        const updatedBudget = {
            ...data.budget,
            monthlyExpenses: data.budget.monthlyExpenses.filter(exp => exp.id !== id)
        };
        updateBudget(updatedBudget);
    };
