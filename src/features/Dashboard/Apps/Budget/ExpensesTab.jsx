...
const ExpensesTab = ({ expenses, smallApp }) => {
  ...
  const handleUpdateExpense = (id, field, value) => {
    const updatedValue =
      typeof value === "string" && field === "cost"
        ? parseFloat(value) || 0
        : value;
    ...
    <tr key={exp.id}>
      ...
      <td>
        <select
          value={exp.category}
          onChange={(e) =>
            handleUpdateExpense(exp.id, "category", e.target.value)
          }
          className={
            `${tableStyles.tableSelect} ` +
            (exp.category === "required"
              ? tableStyles.required
              : exp.category === "flexible"
              ? tableStyles.flexible
              : tableStyles.nonessential)
          }
        >
          <option value="required" className="required">
            Required
          </option>
          <option value="flexible" className="flexible">
            Flexible
          </option>
          <option value="non-essential" className="nonessential">
            Non-essential
          </option>
        </select>
      </td>
  ...
  const renderNewExpenseRow = () => (
    <tr>
      ...
      <td>
        <button className={tableStyles.addButton} onClick={handleAddExpense}>
          Add
        </button>
      </td>
    </tr>
  ...
  return (
    <Section className={tableStyles.tableSection} header={expensesHeader}>
